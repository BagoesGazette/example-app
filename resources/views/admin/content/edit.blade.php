                        ...
                        <div class="card-body">
                            <div class="row gy-3">
                                <div class="form-group">
                                    ...
                                    <select name="menu_id" class="form-control select2 @error('menu_id') is-invalid @enderror">
                                        ...
                                        @foreach ($menu as $item)
                                                <option value="{{ $item->id }}" @if($item->id === $detail->menu_id) selected @endif>{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('menu_id')
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="content" class="form-control @error('content') is-invalid @enderror" id="editor">{{ old('content', $detail->content) }}</textarea>
                                    @error('content')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
